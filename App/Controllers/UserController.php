            ...
            if (move_uploaded_file($tmpFilePath, $destinationPath)) {
                ...
                $successMessage = '
                    <div role="alert" class="fixed bottom-4 right-4 bg-green-500 text-white px-4 py-2 rounded">
                        ...
                        <span>Has pujat i actualitzat la foto de l\'alumne</span>
                    </div>
                ';
...
/**
 * Pujar una foto de perfil des d'un fitxer Base64.
 *
 * @return \Emeset\Http\Response La resposta HTTP amb les dades actualitzades.
 */
public function pujarFotoDesDeFitxerBase64($request, $response, $container)
{
    // Assegura't que s'ha enviat la informació de la imatge en Base64
    if (isset($_POST['foto'])) {
        $imatgeBase64 = $_POST['foto'];

        // Decodificar la imatge Base64
        $dadesImatge = base64_decode(preg_replace('#^data:image/\w+;base64,#i', '', $imatgeBase64));

        // Generar un nom d'arxiu únic
        $nouNomFitxer = uniqid('imatge_') . '.png';

        // Ruta de destinació per desar la imatge
        $rutaDestinacio = "img/" . $nouNomFitxer;

        // Desar la imatge al servidor
        if (file_put_contents($rutaDestinacio, $dadesImatge)) {
            // Èxit al desar la imatge
            $missatgeExit = '<div role="alert" class="fixed bottom-4 right-4 bg-green-500 text-white px-4 py-2 rounded">
            ...
            $modelAlumnes = $container["\App\Models\alumnesPDO"];
            $modelPujadaImatge = $container["\App\Models\alumnesPDO"];

            $idAlumne = $_POST['id_alumne']; 
            $idUsuari = $_SESSION["id_usuari"];

            // Desactivar totes les fotos de l'usuari
            $modelPujadaImatge->desactivarFotosUsuari($idAlumne);

            // Pujar la nova foto de l'usuari
            $modelAlumnes->pujarFotoDesDeFitxer($idAlumne, $rutaDestinacio, 'activa');
        } else {
    ...
    $idUsuari = $_SESSION["id_usuari"];
    $modelFotos = $container["\App\Models\alumnesPDO"];
    $alumnes = $container["\App\Models\alumnesPDO"];
    $alumnes = $alumnes->getAlumnesPerProfessor($idUsuari);
    $foto = $modelFotos->obtenirFotoSeleccionadaUsuari($idUsuari);

    // Configurar les dades per actualitzar la pàgina d'alumnes
    $response->set("alumnes", $alumnes);
    $response->set("foto", $foto);
    $response->SetTemplate("alumnes.php");
